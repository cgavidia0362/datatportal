...
async function initSettingsTab() {
  ...
  const uploadInput = document.getElementById('btnUploadDealersCSV');
    ...
    uploadInput.addEventListener('change', function(e) {
      ...
      Papa.parse(file, {
        ...
        complete: function(results) {
          console.log('[Upload] ✓ Parse complete! Rows:', results.data.length);
          console.log('[Upload] ✓ First row:', results.data[0]);
          ...
          processUpload(results.data, e.target);
        },
        error: function(err) {
  ...
  async function processUpload(rows, inputElement) {
    let updated = 0;
    let added = 0;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const fi = (row['FI Type'] || '').trim();
      const rep = (row['Rep'] || '').trim();
      
      if (!name || !state || !fi) continue;
      
      try {
        const { error: updateErr, count } = await window.sb
          .from('master_dealers')
          .update({ rep: rep || null })
        ...
        if (!updateErr && count > 0) {
          updated++;
        } else if (!updateErr && count === 0) {
          const { error: insertErr } = await window.sb
            .from('master_dealers')
            .insert({ dealer_name: name, state: state, fi: fi, rep: rep || null });
          if (!insertErr) added++;
        }
      } catch (err) {
        console.error('[Upload] Error on row', i, ':', err);
      }
      
      if ((i + 1) % 100 === 0) {
        console.log(`[Upload] Progress: ${i + 1}/${rows.length}`);
      }
    }
    
    console.log('[Upload] ✓ Complete! Updated:', updated, 'Added:', added);
...
function filterMasterDealers() {
  ...
  tbody.innerHTML = filtered.map(dealer => `
    <tr class="border-t hover:bg-gray-50">
      ...
      <td class="px-3 py-2" id="cif-cell-${dealer.dealer_id}">
        ${dealer.cifnumber
          ? `<span class="font-mono text-xs text-gray-700">${dealer.cifnumber}</span>
             <button class="ml-2 text-xs text-blue-500 hover:underline" onclick="window.editCif('${dealer.dealer_id}','${dealer.cifnumber}')">Edit</button>`
          : `<button class="text-xs text-orange-500 hover:underline font-medium" onclick="window.editCif('${dealer.dealer_id}','')">+ Add CIF</button>`
        }
      </td>
  ...
  window.saveCif = async function(dealerId) {
    ...
    try {
      ...
      const { error } = await (sb||window.sb).from('master_dealers').update({ cifnumber: newCif }).eq('dealer_id', dealerId);
      if (error) throw error;
      // Update local cache
      if (window.currentMasterDealers) {
        const d = window.currentMasterDealers.find(x=>x.dealer_id===dealerId);
